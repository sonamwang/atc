...
For direct production serving, ATC accepts `ATC_TLS_CERT_FILE`, `ATC_TLS_KEY_FILE`, `ATC_TLS_CLIENT_CA_FILE`, and `ATC_REQUIRE_MTLS=true`. This enables TLS 1.3 and requires a verified client certificate on every connection. Agent and operator clients accept an explicit trust bundle plus client certificate/key flags (or `ATC_TLS_CA_FILE`, `ATC_CLIENT_TLS_CERT_FILE`, and `ATC_CLIENT_TLS_KEY_FILE`). Certificate identities should still be mapped to organizational tenancy before a deployment is considered multi-tenant production ready.

`ATC_OPERATOR_CREDENTIALS_FILE` enables named RBAC credentials from a private regular JSON file. It rejects symlinks and group/world-readable files, retains token hashes only, and evaluates `viewer`, `operator`, and `admin` permissions independently from bootstrap enrollment and agent tokens. `ATC_OPERATOR_TOKEN` remains a legacy single-admin option. Operator actor names, not tokens, are retained in renewal/revocation audit events.
...
Development CA material, bootstrap credentials, and bearer-agent tokens are development-only facilities. Production deployment requires secret management, mutual TLS, monitoring, independent security review, and a tenant/identity design appropriate to the organization.
