...
`POST /api/v1/agents/{id}/revoke` disables the agent credential without removing its inventory or audit history. It requires an `admin` operator credential (or local-development mode) and `X-ATC-Confirm: revoke`. Repeating a revocation is safe.
...
`POST /api/v1/certificates/{id}/renew` and `POST /api/v1/certificates/{id}/rotate` create a pending renewal job only. They require an `operator` or `admin` bearer credential, `X-ATC-Confirm: renewal`, and a constrained `Idempotency-Key`. Repeating the same request returns the original job; a second active job for the same certificate is rejected. No certificate, key, or service configuration is changed by these endpoints; an owning agent executes its trusted local target configuration.
...
`GET /api/v1/whoami` returns the authenticated operator's configured name and role. `GET /api/v1/ct/{domain}` is available when `ATC_CT_MONITOR_URL` is configured and returns bounded, normalized public CT lookup entries for one valid DNS name.
...
When `ATC_OPERATOR_CREDENTIALS_FILE` is configured, reads require a `viewer`, `operator`, or `admin` bearer token; renewal/rotation requires `operator` or `admin`; revocation requires `admin`. `ATC_OPERATOR_TOKEN` is a legacy single-admin option. Without either setting, endpoints remain in local-development mode. The dashboard accepts an access token in its form and keeps it only in the browser session.
